...
def base_url(request):
    return os.getenv("TEST_BASE_URL") or request.config.getini("base_url")


def test_homepage_loads(page: Page, base_url):
    _navigate_to_homepage(page, base_url)


def test_signup(page: Page, base_url):
    _navigate_to_homepage(page, base_url)
    _create_account(page, "pw_testing@example.com", base_url)

    # go to the edit account view to check credits
    page.goto(f"{base_url}/accounts/edit-account/")
    expect(
        page.locator(
...
def test_upgrade_to_individual(page: Page, base_url):
    _navigate_to_homepage(page, base_url)
    _create_account(page, "pw_individual@example.com", base_url)

    _navigate_to_homepage(page, base_url)

    _upgrade_subscription(page, "individual", base_url)


def test_upgrade_to_team(page: Page, base_url):
    _navigate_to_homepage(page, base_url)
    _create_account(page, "pw_team@example.com", base_url)

    _navigate_to_homepage(page, base_url)

    _upgrade_subscription(page, "team", base_url)


def test_upgrade_individual_to_team(page: Page, base_url):
    _navigate_to_homepage(page, base_url)
    _create_account(page, "pw_upgrade@example.com", base_url)
...
def test_invite_team_member(page: Page, base_url):
    _navigate_to_homepage(page, base_url)
    _create_account(page, "pw_team-owner@example.com", base_url)
...
@pytest.mark.django_db
def test_downgrade_team_to_individual(page: Page, base_url):
    _navigate_to_homepage(page, base_url)
    _create_account(page, "pw_downgrade@example.com", base_url)
...
def _navigate_to_homepage(page: Page, base_url):
...
def _upgrade_subscription(page: Page, tier: str, base_url):
    ...
    expect(page).to_have_url(f"{base_url}/accounts/edit-account/", timeout=30000)
